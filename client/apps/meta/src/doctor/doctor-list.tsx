...
const UserList = () => {
	...
	const DoctorName = GetDoctorDataFn();
	console.log(DoctorName.data, "Doctor data....");
	const deleteDoctor = DeleteDoctorDataFn();
	...
	const filteredData = (DoctorName.data || []).filter((item) =>
		item.doctor_name
			?.toString()
			.toLowerCase()
					...
					<Center>
						...
						>
							{filteredData?.map((items) => (
								<div key={items.id}>
									<DashboardListCard
										heading_1={"Doctor name"}
										result_1={items.doctor_name}
										heading_2={"Experience:"}
										result_2={items.experience}
