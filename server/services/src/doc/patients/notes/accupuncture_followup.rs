...
use patient::dto::notes::notes_types::AcupunctureFollowUp;
use patient::dto::notes::notes_types::NoteType;
use serde_json::Value;
use utils::store_helper::note_store;
use uuid::Uuid;

pub async fn add_acupuncturefollowup(
    params: AcupunctureFollowUpAdd,
    app_state: AppState,
    ...
    let command = CreateNote {
        ...
        note: NoteType::AcupunctureFollowUp(AcupunctureFollowUp {
            ...
            tcm_exam: params.tcm_exam.clone(),
            assessment: params.assesment.clone(),
            plan: params.plan.clone(),
            treatment: params.treatment.clone(),
        }),
    ...
    info!("Processing Acupuncture follow up events...");
    let res =
    ...
    match res {
        anyhow::Result::Ok(_) => {
            info!("Acupuncture follow up added successfully.");
            serde_json::to_value(row).map_err(ErrorData::from)
        }
        Err(err) => {
            error!(
                "Error occurred while adding Acupuncture follow up: {:#?}",
                err
            );
            Err(ErrorData {
                message: String::from("ACUPUNCTURE_FOLLOW_UP_NOT_ADDED"),
                data: Value::Null,
                code: -32600,
            })
        }
