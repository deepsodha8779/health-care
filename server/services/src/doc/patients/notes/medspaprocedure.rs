...
use patient::dto::notes::notes_types::MedSpaProcedure;
use patient::dto::notes::notes_types::NoteType;
use serde_json::Value;
use utils::store_helper::note_store;
use uuid::Uuid;

pub async fn add_medspaprocedure(
    params: MedSpaProcedureAdd,
    app_state: AppState,
    ...
    let command = CreateNote {
        ...
        note: NoteType::MedSpaProcedure(MedSpaProcedure {
    ...
    info!("Processing medspaprocedure events...");
    let res =
    ...
    match res {
        anyhow::Result::Ok(_) => {
            info!("MedSpaProcedure added successfully.");
            serde_json::to_value(row).map_err(ErrorData::from)
        }
        Err(err) => {
            error!("Error occurred while adding MedSpaProcedure: {:#?}", err);
            Err(ErrorData {
                message: String::from("MED_SPA_PROCEDURE_NOT_ADDED"),
                data: Value::Null,
                code: -32600,
            })
        }
