...
pub async fn add_officeform(
    ...
    let command = CreateNote {
        ...
        note: NoteType::OfficeForm(OfficeForm {
    ...
    info!("Processing officeForm events...");
    let res =
    ...
    match res {
        anyhow::Result::Ok(_) => {
            info!("OfficeForm added successfully.");
            serde_json::to_value(row).map_err(ErrorData::from)
        }
        Err(err) => {
            error!("Error occurred while adding OfficeForm: {:#?}", err);
            Err(ErrorData {
                message: String::from("OFFICE_FORM_NOT_ADDED"),
                data: Value::Null,
                code: -32600,
            })
        }
