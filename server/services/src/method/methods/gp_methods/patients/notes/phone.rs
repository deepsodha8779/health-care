...
pub enum PhoneMethods {
    Add(PhoneAdd, Option<String>),
}

impl Rpc for PhoneMethods {
        ...
        Self: Sized,
    {
        ...
        let res = if let Some((first, _elements)) = names.split_first() {
            match *first {
                "Add" => {
                    let input = serde_json::from_value::<PhoneAdd>(data[0].clone())
                        .map_err(|_| ErrorData::std(-32601))?;
                    Ok(PhoneMethods::Add(input, None))
                }
    ...
    fn to_rpc(&self, namespace: &str) -> Result<Request, ErrorData> {
        match self {
            PhoneMethods::Add(input, _) => create_request(namespace, "Add", input),
        }
    }
}
